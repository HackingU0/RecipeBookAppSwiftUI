...
class RecipeManager: ObservableObject {
    ...
    init() {
        ...
        if recipes.isEmpty {
            // Add your predefined recipes here
            recipes = [
                Recipe(title: "Classic Margherita Pizza", headline: "Lunch", ingredients: ["Pizza dough", "Tomatoes", "Fresh mozzarella", "Basil", "Olive oil"], instructions: "Start by preheating your oven...", imageName: "pizza"),
   ...
   func loadFromUserDefaults() {
        if let savedItems = UserDefaults.standard.data(forKey: "recipes") {
            if let decodedItems = try? JSONDecoder().decode([Recipe].self, from: savedItems) {
